    ...
    while (try inspector.next_syscall()) |position_context| {
        ...
        switch (position_context) {
            ...
            .pre_call => |context| {
                ...
                const pathname = switch (@intToEnum(os.SYS, context.registers.orig_syscall)) {
                ...
                warn("pid = {}, '{}' path = '{}'\n", .{ context.pid, @tagName(@intToEnum(os.SYS, context.registers.orig_syscall)), pathname });
            ...
            .post_call => |context| {
                warn("pid = {}, '{}' = {}\n\n", .{ context.pid, @tagName(@intToEnum(os.SYS, context.registers.orig_syscall)), @intCast(isize, context.registers.syscall_then_result) });
                try inspector.resume_tracee(context.pid);
            },
        }
    }
